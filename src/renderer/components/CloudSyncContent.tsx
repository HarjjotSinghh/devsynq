...
interface CloudSyncContentProps {
    onClose?: () => void;
    onToast: (message: string) => void;
    className?: string; // Allow custom styling wrapper
}

export const CloudSyncContent: React.FC<CloudSyncContentProps> = ({
    onClose,
    onToast,
    className
}) => {
    ...
    const handleUpgrade = async () => {
        setIsLoading(true);
        try {
            const result = await window.electronAPI.cloudSyncCreateCheckoutSession();
            if (result.success && result.checkoutUrl) {
                await window.electronAPI.openExternal(result.checkoutUrl);
                onToast('Opening checkout page...');
            } else {
                onToast(result.error || 'Failed to start upgrade');
            }
        } catch (error) {
            console.error('Upgrade error:', error);
            onToast('Failed to start upgrade');
        }
        setIsLoading(false);
    };
    ...
    return (
        <div className={`cloud-sync-inner-content ${className || ''}`}>
            {/* Header if onClose is provided (Modal Mode) or just title */}
            {onClose && (
                            ...
                            <div className="status-content">
                                <div className="account-info">
                                    ...
                                    <div className="info-row">
                                        ...
                                        <span className="value">
                                            {config.planType === 'pro' ? '🏆 Pro' : 'Free'}
                                        </span>
                                    </div>
                                    {config.planType !== 'pro' && (
                                        <div className="upgrade-section" style={{ marginTop: '10px', textAlign: 'center' }}>
                                            <button
