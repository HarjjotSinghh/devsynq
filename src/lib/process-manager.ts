...
async function getWindowsProcesses(): Promise<RunningIDE[]> {
    ...
    try {
        ...
        const psCommand = `Get-Process | Where-Object { $_.MainWindowTitle -ne '' -or $_.Name -match 'Code|Cursor|Windsurf|Zed|WebStorm|idea|pycharm|Trae|Replit|Cody' } | Select-Object Id, Name, Path, CPU, WorkingSet64, StartTime | ConvertTo-Json -Compress`;
...
async function getMacProcesses(): Promise<RunningIDE[]> {
    ...
    try {
        ...
        const output = await execAsync("ps aux | grep -E 'Cursor|Code|Windsurf|Zed|webstorm|idea|pycharm|Trae|Replit|Cody' | grep -v grep");
